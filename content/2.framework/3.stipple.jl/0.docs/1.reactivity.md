...
2. The reactive variables watched by a handler are passed to the handler by value, not by reference. 
    ...
    ```
    See [here](/framework/stipple.jl/docs/reactivity#under-the-hood-reactive-models-and-observables) to learn how the reactive model works.
...
Reactive variables are stored as fields in [a struct named `__model__`](/framework/stipple.jl/docs/reactivity#under-the-hood-reactive-models-and-observables), which is only exposed inside the `@app` block. The content of a model can be printed from a handler like this:
