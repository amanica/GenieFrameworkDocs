...
3. In the Julia REPL, enter `Pkg` mode by typing ], then activate the current directory:

```julia
pkg> activate .
```

4. In the Julia REPL, enter `Pkg` mode by typing ], then install `GenieFramework.jl`:
...
5. Create a file named app.jl file with the following content:
...
6. Exit `Pkg` mode by pressing backspace. Then, in the Julia REPL, load and run your app:
...
7. Open a web browser and go to http://localhost:8000. You should see your "Hello, Genie!" message.
